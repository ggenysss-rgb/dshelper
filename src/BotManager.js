...
class BotManager {
    ...
    getUserConfig(userId) {
        ...
        const config = {
            ...
            staffRoleIds: row.staff_role_ids ? JSON.parse(row.staff_role_ids) : [],
            ...
            priorityKeywords: row.priority_keywords ? JSON.parse(row.priority_keywords) : { high: [], medium: [] },
            ...
            rateLimitMs: row.rate_limit_ms || 1500,
    ...
    async startBot(userId) {
        ...
        if (!config || !config.discordToken || !config.tgToken || !config.tgChatId) {
