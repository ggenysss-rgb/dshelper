...
// Send opcode 14 — Guild Subscribe (Lazy Request) — required for user tokens to receive MESSAGE_CREATE
function sendLazyRequest(bot, guildId, channelIds) {
    ...
    try {
        ...
        bot.log(`📡 Lazy Request: subscribed to ${channelIds.length} channels in guild ${guildId}`);
    } catch (e) {
        bot.log(`❌ Lazy Request error: ${e.message}`);
    }
}
